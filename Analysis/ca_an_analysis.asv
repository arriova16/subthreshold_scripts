...
for i = 1:length(ca_an_struct)
    ...
    %leaving stim alone and shuffling indentor amp tied to responses
        ...
        c_dif = ca_an_struct(i).ResponseTa;
        % mech_u = unique(ca_an_struct(i).ResponseTable.IndentorAmp);
        %uses same perm'd rows and using it to tie together the response
        %with the mech amp
        

        %getting conditions for each of the mech amps
        % perm_response =ca_an_struct(i).ResponseTable.Response(randomindices);
        % perm_mech = ca_an_struct(i).ResponseTable.IndentorAmp(randomindices);
        
        %creating a new response table with the perm response and perm mech amp 

        % perm_RT = ca_an_struct(i).ResponseTable;
        % perm_RT.Response = perm_response;
        % perm_RT.IndentorAmp = perm_mech;
        % ca_an_struct(i).perm_RT = perm_RT;
        % 
        % [dt_perm, dp_perm] = AnalyzeResponseTable(ca_an_struct(i).perm_RT);
...
for i = 1:length(ca_an_struct)
   for c = 1
      ...
      figure; hold on
        ...
        plot(qq, ca_an_struct(p).yf_cont{c})
        plot(qq, ca_an_struct(p).yf_stim{c})
        plot([0 ca_an_struct(p).mech_thresh_cont{c} ca_an_struct(p).mech_thresh_cont{c}], [threshold, threshold, -1],'Color',rgb(69, 90, 100),'LineStyle','--' )
