...
for p = 1:length(ca_an_struct)  
    ...
    half_length = floor(num_trials/2);

    mech_u = unique(ca_an_struct(p).ResponseTable.IndentorAmp);
    no_stim = find(ca_an_struct(p).ResponseTable.StimAmp == 0);
    stim = find(ca_an_struct(p).ResponseTable.StimAmp ~= 0);
    uniqStimAmp = unique(ca_an_struct(p).ResponseTable.StimAmp);
    qq = linspace(mech_u(1), mech_u(end)*2);
    ...
    for dm = 1:num_perm
        ...
        [p1detected, p2detected] = deal(zeros(size(mech_u)));

        for a = 1:length(mech_u)
            p1u_idx = ca_an_struct(p).ResponseTable.IndentorAmp(p1_idx) == mech_u(a);
            p1detected(a) = sum(strcmp(ca_an_struct(p).ResponseTable.Response(p1_idx(p1u_idx),:), 'correct')) / sum(p1u_idx);
            p2u_idx = ca_an_struct(p).ResponseTable.IndentorAmp(p2_idx) == mech_u(a);
            p2detected(a) = sum(strcmp(ca_an_struct(p).ResponseTable.Response(p2_idx(p2u_idx),:), 'correct')) / sum(p2u_idx);
            if mech_u(a) == 0
        ...
        [~, coeffs1, ~,~,~,~] = FitSigmoid(mech_u, p1detected, 'Constraints', [0, 300; -5, 5]);
        m1 = Sigmoid2MechThreshold(coeffs1, qq, threshold);
        [~, coeffs2, ~,~,~,~] = FitSigmoid(mech_u, p2detected, 'Constraints', [0, 300; -5, 5]);
        m2 = Sigmoid2MechThreshold(coeffs2, qq, threshold);
        
        null_delta_threshold(dm) = abs(m1 - m2);
        ca_an_struct(p).Per_threshold = null_delta_threshold(dm);
      ca_an_struct(p).Bootp = sum(null_delta_threshold > delta_thresholds) / num_perm ;
